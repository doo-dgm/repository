...
LEFT JOIN status AS stsp ON pqr.statusId = stsp.id
LEFT JOIN person AS perq ON pqr.personId = perq.id
LEFT JOIN document AS docp ON perq.documentId = docp.id
LEFT JOIN sector AS secp ON pqr.sectorId = secp.id
LEFT JOIN Municipality AS munp ON secp.municipalityId = munp.id
LEFT JOIN State AS stap ON munp.stateId = stap.id
LEFT JOIN Country AS coup ON stap.countryId = coup.id
LEFT JOIN risk AS risp ON pqr.riskId = risp.id
		""";
